...
Uso recomendado en Colab o entorno local con GPU:
    pip install torch torchvision opencv-python scikit-image matplotlib pandas tqdm pillow requests streamlit timm==0.4.12
...
def apply_styles() -> None:
    ...
    .hero-wrap {{
        background: linear-gradient(135deg, #07111B 0%, #10172A 52%, #0B1F23 100%);
    ...
    .hero-title {{
        ...
        line-height: 1.05;
        font-weight: 700;
        background: linear-gradient(90deg, {CYAN} 0%, {VIOLET} 52%, {MINT} 100%);
    ...
    .stButton > button {{
        ...
        font-weight: 700;
        font-size: 0.92rem;
        background: linear-gradient(135deg, #00384A, #101633);
        color: {CYAN};
    ...
    .stButton > button:hover {{
        background: linear-gradient(135deg, #004E66, #1A2250);
        color: #9EEBFF;
        border-color: {CYAN};
...
def load_swinir_color_denoising_model(: Path, model_path: Path, device: torch.device) -> torch.nn.Module:
...
def results_to_rows(results: List[ImageResult]) -> List[Dict[str, object]]:
    return [
        {
            "Imagen": r.image_name,
            "PSNR antes": round(r.psnr_input, 4),
            "PSNR despues": round(r.psnr_restored, 4),
            "SSIM antes": round(r.ssim_input, 4),
            "SSIM despues": round(r.ssim_restored, 4),
            "ENL antes": round(r.enl_input, 4),
            "ENL despues": round(r.enl_restored, 4),
            "Resultado": r.analysis,
        }
        for r in results
    ]


def fig_before_after(input_path: Path, restored_path: Path, gt_path: Path):
    input_rgb = read_rgb(input_path)
    gt_rgb = read_rgb(gt_path)
    restored_rgb = resize_like(read_rgb(restored_path), gt_rgb)
    input_rgb = resize_like(input_rgb, gt_rgb)
    diff_input = cv2.absdiff(gt_rgb, input_rgb)
    diff_restored = cv2.absdiff(gt_rgb, restored_rgb)
    ...
    panels = [
        ...
        (restored_rgb, "SwinIR"),
        (gt_rgb, "Ground truth"),
        (diff_input, "Error antes"),
        (diff_restored, "Error despues"),
        (cv2.cvtColor(restored_rgb, cv2.COLOR_RGB2GRAY), "Restaurada gris"),
    ]
...
def build_namespace_from_cfg(cfg: Dict[str, object]) -> argparse.Namespace:
    clean = dict(cfg)
    clean["input_dir"] = clean_optional_path(clean.get("input_dir"))
    clean["model_path"] = clean_optional_path(clean.get("model_path"))
    return argparse.Namespace(**clean)
...
def render_sidebar():
    ...
    default_gt = script_dir / "img"
    default_out = script_dir / "resultados_swinir"
    default_repo = script_dir / "SwinIR"

    with st.sidebar:
        st.markdown(f"""
        <div style='text-align:center; padding:0.9rem 0 0.55rem;'>
            <div style='font-size:1.28rem; font-weight:700;
                        background:linear-gradient(90deg,{CYAN},{VIOLET},{MINT});
                        -webkit-background-clip:text; -webkit-text-fill-color:transparent;
                        background-clip:text;'>SwinIR Studio</div>
            <div style='font-size:0.68rem; color:{MUTED}; letter-spacing:0.1em; text-transform:uppercase;'>
                Reto Transformer
            </div>
        </div>
        <hr style='border-color:{GRID}; margin:0.35rem 0 0.85rem;'>
        """, unsafe_allow_html=True)

        st.markdown("**Entrada**")
        gt_dir = st.text_input("Ground truth", value=str(default_gt))
        generate_defects = st.checkbox("Generar defectos desde GT", value=True)
        input_dir = st.text_input("Imagenes degradadas", value="", disabled=generate_defects)
        max_images = st.slider("Imagenes a evaluar", 3, 10, 3, 1)

        st.markdown("---")
        output_dir = st.text_input("Salida", value=str(default_out))

        st.markdown("---")
        run = st.button("Generar reporte")

    cfg = {
        "input_dir": input_dir,
        "gt_dir": gt_dir,
        "output_dir": output_dir,
        "swinir_repo": default_repo,
        "model_path": None,
        "sigma": 25,
        "generate_defects": bool(generate_defects),
        "defect_type": "speckle",
        "noise_level": 25.0,
...
def preview_dataset(cfg: Dict[str, object]) -> None:
    gt_dir = Path(str(cfg["gt_dir"]))
    st.markdown('<div class="section-title">Entrada</div>', unsafe_allow_html=True)
    if not gt_dir.exists():
        st.warning(f"No se encontro la carpeta: {gt_dir}")
        return

    images = list_images(gt_dir)
    c1, c2, c3 = st.columns(3)
    c1.metric("Ground truth", f"{len(images)}")
    c2.metric("Modo entrada", "Sintetico" if cfg["generate_defects"] else "Pares")
    c3.metric("A evaluar", f"{min(len(images), int(cfg['max_images']))}")


def run_streamlit_pipeline(cfg: Dict[str, object]) -> Dict[str, object]:
    args = build_namespace_from_cfg(cfg)
    set_seed(args.seed)
    ...
    status.info("Verificando dependencias de SwinIR")
    ensure_swinir_python_dependencies()
    bar.progress(0.12)

    status.info("Preparando pares de imagenes")
    ...
    status.info("Ejecutando restauracion con SwinIR")
    ...
    status.info("Calculando metricas y evidencias")
    results = evaluate_all(pairs, restored_dir, evidence_dir)
    csv_path = output_dir / "metricas_reto_transformer.csv"
    conclusions_path = output_dir / "conclusiones_reto_transformer.md"
    write_csv(results, csv_path)
    write_conclusions(results, conclusions_path)
    bar.progress(1.0)
...
def render_results(res: Dict[str, object]) -> None:
    results = res["results"]
    if not results:
    ...
    st.markdown('<div class="section-title">Resultados generales</div>', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Imagenes", f"{len(results)}")
    c2.metric("PSNR promedio", f"{psnr_gain:+.2f} dB")
    c3.metric("SSIM promedio", f"{ssim_gain:+.4f}")
    c4.metric("ENL promedio", f"{enl_gain:+.2f}")
    ...
    st.markdown('<div class="section-title">Comparacion visual</div>', unsafe_allow_html=True)
    pairs = res["pairs"]
    restored_dir = res["restored_dir"]
    options = [Path(gt).name for _, gt in pairs]
    chosen = st.selectbox("Imagen", options)
    selected_pair = next((p for p in pairs if Path(p[1]).name == chosen), pairs[0])
    restored_path = find_restored(restored_dir, selected_pair[0].name)
    if restored_path is None:
        st.warning("No se encontro la imagen restaurada para la seleccion.")
    else:
        st.pyplot(fig_before_after(selected_pair[0], restored_path, selected_pair[1]), use_container_width=True)

    st.markdown(
        f'<div class="path-line">Reporte guardado en: {Path(res["output_dir"]).resolve()}</div>',
        unsafe_allow_html=True,
    )
...
def streamlit_main() -> None:
    if st is None:
        raise RuntimeError("Streamlit no esta instalado. Instala streamlit o usa la interfaz CLI.")
    st.set_page_config(
        page_title="SwinIR Studio",
        page_icon="S",
        layout="wide",
        initial_sidebar_state="expanded",
    )
    apply_styles()
    cfg, run = render_sidebar()

    st.markdown(f"""
    <div class="hero-wrap">
        <div class="hero-title">SwinIR Studio</div>
        <div class="hero-sub">Transformer preentrenado · restauracion de imagenes · PSNR / SSIM / ENL</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if run:
        st.session_state.pop("reto4_results", None)
        try:
            st.session_state["reto4_results"] = run_streamlit_pipeline(cfg)
        except SystemExit as exc:
            st.error(str(exc))
        except Exception as exc:
            st.exception(exc)

    if "reto4_results" in st.session_state:
        render_results(st.session_state["reto4_results"])
